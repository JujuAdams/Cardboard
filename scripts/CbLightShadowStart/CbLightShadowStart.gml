...
function CbLightShadowStart(_index, _setShader = true)
{
    ...
    {
        __CbError("Cannot set shadow mapping as a shadow mapping is already set (", _global.__lightingShadowCurrent, ")");
    }
    ...
    with(_global.__lightingShadowArray[_index])
    {
        if (not __state)
        {
            __CbError("Cannot set shadow mapping surface for light ", _index, " as it has been disabled");
        }
        ...
        if (_setShader)
        {
            ...
            shader_set_uniform_f(shader_get_uniform(__shdCbDepth, "u_fAlphaTestRef"), _global.__alphaTestRef);
            shader_set_uniform_f(shader_get_uniform(__shdCbDepth, "u_vZ"), __near, __far);
        }
