...
/// This function additionally enables use of CbSpriteBillboard*()
/// 
...
function CbRenderStateSet(_width, _height, _fromX, _fromY, _fromZ, _toX, _toY, _toZ, _axonometric = undefined, _alphaTestRef = 128)
{
    __CB_GLOBAL
    ...
    _global.__alphaTestRef = clamp(_alphaTestRef/255, 0, 1);
    
    //Set up GPU state
    gpu_set_ztestenable(true);
    gpu_set_zwriteenable(true);
    gpu_set_cullmode((CB_WRITE_NORMALS && CB_BACKFACE_CULLING)? cull_clockwise : cull_noculling);
    gpu_set_alphatestenable(true);
    gpu_set_alphatestref(_alphaTestRef);
    
    CbViewMatrixSet(_fromX, _fromY, _fromZ, _toX, _toY, _toZ, _axonometric);
    matrix_set(matrix_projection, matrix_build_projection_ortho(_width, _height, __CB_Z_NEAR, __CB_Z_FAR));
}